import numpy as np
from spline_path import calc_2d_spline_interpolation_fixed_length, calc_2d_spline_interpolation_fixed_distance
...
def prenormalize_path(path, num_nodes=100):
    ...
    # distribute the points that need to be added over the subsequences proportionaly to their length
...
def normalize_path_fixed_length(path, num_nodes=20):
    """
    ...
    path = np.array(path)
    x, y = path[:, 0], path[:, 1]
    x, y, yaw, k, travel = calc_2d_spline_interpolation_fixed_length(x, y, num_nodes=num_nodes)
