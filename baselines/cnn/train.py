...
parser = CNNBaseline.add_model_specific_args(parser)

args = parser.parse_args()

def create_logger_name_for_args(args):
    name = ["CNNBaseline", args.dataset]
...
def main(args):
    ...
    if args.test:
        model = CNNBaseline.load_from_checkpoint(args.checkpoint_path)
        trainer.test(model)
    else:
        model = CNNBaseline(args)
        trainer.fit(model)
        model = CNNBaseline.load_from_checkpoint(os.path.join(args.save_dir, name + ".ckpt"))
        trainer.test(model)
