...
def main(args):
    ...
    for bidx, data in enumerate(tqdm(test_loader)):
        layout, gt_path_nodes, start_pos, command_embedding = data["x"].float(), data["path"], data["start_pos"], \
        ...
        path_nodes = model.forward(layout, start_pos, command_embedding)
        loss = criterion(path_nodes, gt_path_nodes)
        loss = loss.mean()

        path_unnormalized = start_pos.unsqueeze(2) + path_nodes.cumsum(
            dim=2)  # (B, self.num_path_hyps, self.num_path_nodes, 2)
        ...
        ).norm(2, dim=-1).mean(dim=-1).min(-1)[0].unsqueeze(1)
        ...
        path_unnormalized = path_unnormalized[0, 0, :, :].unsqueeze(0).cpu().numpy()
        gt_path_unnormalized = gt_path_unnormalized[0, :, :, :].cpu().numpy()
    ...
    print(f"Median endpoint ADE: {all_endpoint_ades.median().item():.2f} m")
