...
def create_logger_name_for_args(args):
    name = ["LSTMBaselineFixedPathLength", args.dataset]
    ...
    name.append("embedding_dim_{}".format(args.embedding_dim))
    name.append("hidden_dim_{}".format(args.hidden_dim))
...
def main(args):
    ...
    if args.test:
        model = LSTMBaseline.load_from_checkpoint(args.checkpoint_path)
        trainer.test(model)
    else:
        model = LSTMBaseline(args)
        trainer.fit(model)
        model = LSTMBaseline.load_from_checkpoint(os.path.join(args.save_dir, name + ".ckpt"))
        trainer.test(model)
