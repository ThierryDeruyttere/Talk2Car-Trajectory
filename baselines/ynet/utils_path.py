...
def draw_paths_heatmap_t2c(
    ...
    red_rect = np.ones(heatmap.shape)[np.newaxis, :] * 255
    red_rect = np.concatenate(
        [
            red_rect,
            np.zeros(red_rect.shape),
            np.zeros(red_rect.shape)
        ],
    )
    red_rect = Image.fromarray(red_rect.transpose(1, 2, 0).astype(np.uint8))
    # hm = Image.fromarray(heatmap / heatmap.max() * 255).convert("LA")
    red_rect.putalpha(Image.fromarray(heatmap / heatmap.max() * 255).convert("L"))
    img = Image.alpha_composite(img.convert("RGBA"), red_rect.resize((1200, 800)))
    blended = draw_objects_and_endpoints_on_img(
        img,
        paths,
        gt_paths,
...
def draw_heatmap_t2c(
    objects, gt_object, img_path, endpoints, log_px_pred, X, Y, save_path=None, alpha=0.4, levels=20, det_objs=None
):
